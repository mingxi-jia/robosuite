...
class Phantom(SingleArmEnv):
    ...
    def __init__(
        ...
        self.robot_offset = np.array(robot_offset)
        self.table_offset = np.array((0, 0, 0.8)) + np.array(table_offset)
    ...
    def _load_model(self):
        ...
        self.robots[0].robot_model.set_base_xpos(xpos + self.robot_offset)
        ...
        # Modify agentview camera pose when caller provides one
        if self.camera_pos is not None:
            cam_attribs = {}
            if self.camera_sensorsize is not None:
                cam_attribs = {
            ...
            mujoco_arena.set_camera(
                ...
                **({'camera_attribs': cam_attribs} if cam_attribs else {}),
            )
